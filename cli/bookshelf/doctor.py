...
def _warn_deployment_posture(report: DoctorReport, health_data: dict[str, Any], api_url: str) -> None:
    ...
    # 权限阶段 1/4：匿名书架（C=lan_shared / B=explicit_public）配置一致性——
    # 两种模式的信任门控相同，无可信 CIDR 时除回环/可信代理外一律降级为登录页
    _anon_mode = health_data.get("anonymous_catalog_mode")
    if _anon_mode in ("lan_shared", "explicit_public") and not health_data.get("trusted_lan_configured"):
        report.warnings.append(
            f"匿名书架 {_anon_mode} 已开启但未配置 TRUSTED_LAN_CIDRS：除回环与可信代理链外，"
